...
export class NftService {
  //Replace this to the frontend?
  //Should use localhost ipfs node instead of infura
  ...
  // Not sure if this address currently works(Deployed it on my own node)
  ...
  async createNft(data: any): Promise<string> {
    // 1. Save the data to IPFS
    
    const ipfsResult = await ipfs.add(JSON.stringify(data));
    const ipfsHash = ipfsResult.path;

    // Mint the NFT
    // Hardcoded for my metamask wallet
    const tx = await this.contract.mintNft('0x98D67630AC1ab8dfe62BE4AEde05Fd204B190e4D', ipfsHash);
    await tx.wait();
