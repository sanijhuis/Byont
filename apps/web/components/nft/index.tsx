...
const ipfs = create({ host: "localhost", port: 8001, protocol: "http" });
...
// Might need to replace with your own contract address
...
export default function NftMinter() {
  ...
  useEffect(() => {
    if (provider) {
      setContract(
        new ethers.Contract(contractAddress, contractAbi, provider.getSigner())
      );
    }
  }, [provider]);
  ...
  async function handleMintNft() {
    const data = { name: "Test Scan", result: "clean", date: "2023-05-25" };
    const result = await createNft(data);
